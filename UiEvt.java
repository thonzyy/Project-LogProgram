...
//	private String filePath;
	...
	public void openFiledialog() {
		try {
			if (!ui.getjtStart().getText().isEmpty() && Integer.parseInt(ui.getjtStart().getText()) < 1) {
	...
	public void reportOpen() throws NullPointerException, IOException {
		if(ui.getLfe().isAuthority()) {
			openFiledialog();
			logio.writeReport();
			JOptionPane.showMessageDialog(ui,"파일 저장 완료하였습니다.");
		} else {
	...
	public void actionPerformed(ActionEvent ae) {
		...
		if( ae.getSource() == ui.getjbReport()) {
			try {
				...
				JOptionPane.showMessageDialog(ui,"파일 저장 실패!\n다시 시도 해주세요.");
